...
int parseParameters(int argc, char* argv[]) {
	...
	for (int i = 1; i < argc; i+=2) {
		...
		if (string(argv[i]) == "-seed") { //dummy right now
		  if (i + 1 < argc) { // Make sure we aren't at the end of argv!
		    stoi(argv[i + 1],NULL); // Increment 'i' so we don't get the argument as the next argv[i].
		  } else { // Uh-oh, there was no argument to the destination option.
		    std::cerr << "-seed option requires an integer argument." << std::endl;
		    return -1;
		  }
		}
		if (string(argv[i]) == "-o") {
		  if (i + 1 < argc) { // Make sure we aren't at the end of argv!
		    outDir = string(argv[i + 1]); // Increment 'i' so we don't get the argument as the next argv[i].
		  } else { // Uh-oh, there was no argument to the destination option.
		    std::cerr << "-o option requires an integer argument." << std::endl;
		    return -1;
		  }
		}
	}
...
// could make this use iterators
double dot(vector<NeuronState_v_n_a_s> vec, int start, int end) {
...
int writePermutationToFile (vector<int> &order, const string fileNameStr){

	ofstream myfile(fileNameStr);
	// unsigned int nNeurons = order.size();
...
int main(int argc, char* argv[]) {
	...
	vector<boost::array<double, 3> > diffs; //difference between inner product and reordered; Exc, Inh, (excitatory) value

	if (parseParameters(argc, argv)<0)
		exit(0);

	vector<int> order(nNeurons);
	...
	//compute fixed permutation for ordering comparison; praise move semantics; excitatory only for now, inhibitory is same
	for (int i = 0; i < nNeurons; ++i) order[i] = i;
	std::shuffle(order.begin(), order.begin() + nExcNeurons, std::mt19937{std::random_device{}()});
	std::shuffle(order.begin() + nExcNeurons, order.end(), std::mt19937{std::random_device{}()});
	...
	if (SAVE_SIMULATION) {
		...
		string fileNameStr = outDir + sdt + ".txt";
		...
		fileNameStr = outDir + sdt + "Epis.txt";
		...
		fileNameStr = outDir + sdt + "Iapp.txt";
		...
		fileNameStr = outDir + sdt + ".m";
		mSpikeTrain->printToMatlabFile(fileNameStr);

		fileNameStr = outDir + sdt + "FP.txt";
		writeFPErrorsToFile(diffs, fileNameStr);

		fileNameStr = outDir + sdt + "Ord.txt";
		writePermutationToFile(order, fileNameStr);
	}
