...
cpp_dec_float_100 bigAtotExc, bigAtotInh; //for FP
...
boost::array<double, 2> twoSum(double a, double b) {
  double x = a + b;
  double z = x - a;
  return {x, a - (x - z) + (b - z)};
}

boost::array<double, 2> split(double a) {
  double c = (1<<27 + 1) * a;
  double x = c - (c - a);
  return {x, a - x};
}

boost::array<double, 2> twoProduct(double a, double b) {
  double x = a * b;
  boost::array<double, 2> a1_a2 = split(a);
  boost::array<double, 2> b1_b2 = split(b);
  return {x, a1_a2[1] * b1_b2[1] - (((x - a1_a2[0] * b1_b2[0]) - a1_a2[1] * b1_b2[0]) - a1_a2[0] * b1_b2[1])};
}

double dot2(int start, int end) { //goes from start to end-1
  if (start >= end) return 0;
  boost::array<double, 2> temp = twoProduct(network[start][2], network[start][3]);
  double p = temp[0], s = temp[1], h, q, r;
  for (int i = start + 1; i < end; ++i) {
    temp = twoProduct(network[i][2], network[i][3]);
    h = temp[0];
    r = temp[1];
    temp = twoSum(p, h);
    p = temp[0];
...
int main(int argc, char* argv[]) {
	...
	vector< boost::array<double, 3> > errsFP;
	...
	if (SAVE_SIMULATION) {
		...
		writeFPErrorsToFile(errsFP, fileNameStr);
	}
