...
vector<double> iapp_i(nNeurons); 				// for Inhibitory neurons
double iappj;
...
using bigType = number<cpp_dec_float<100>, et_off>;
...
// Function to write a .txt file with a time series of the floating point errors in the dot products (excitatory and inhibitory)
int writeFPErrorsToFile (vector< boost::array<double, 3> > &v, string const fileNameStr){
...
template<typename T>
boost::array<T, 2> twoSum(T a, T b) {
  T x = a + b;
  T z = x - a;
  return {x, a - (x - z) + (b - z)};
}
...
template<typename T>
boost::array<T, 2> twoProduct(T a, T b) {
  T x = a * b;
  boost::array<T, 2> a1_a2 = split(a);
  boost::array<T, 2> b1_b2 = split(b);
  return {x, a1_a2[1] * b1_b2[1] - (((x - a1_a2[0] * b1_b2[0]) - a1_a2[1] * b1_b2[0]) - a1_a2[0] * b1_b2[1])};
}

//would be NeuronState_v_n_a_s
template<typename T>
T dot2(vector<NeuronState_v_n_a_s> &vec, int start, int end) { //goes from start to end-1
  if (start >= end) return 0;
  boost::array<T, 2> temp = twoProduct<T>(vec[start][2], vec[start][3]);
  T p = temp[0], s = temp[1], h, q, r;
  for (int i = start + 1; i < end; ++i) {
    temp = twoProduct<T>(vec[i][2], vec[i][3]);
    h = temp[0];
    r = temp[1];
    temp = twoSum(p, h);
    p = temp[0];
...
int main(int argc, char* argv[]) {
	...
	vector<boost::array<double, 3> > diffs; //difference between inner product and reordered; 

	if (parseParameters(argc, argv)<0)
		exit(0);

	vector<NeuronState_v_n_a_s> permuted(nNeurons);
	...
	while (burstCount<maxNumBurst and t<=maxTimeSimulation) {
		...
		// }
		bigAtotExc = dot2<bigType>(network, 0, nExcNeurons);

		// Synaptic drive from inhibitory cells
		// for ( n = nExcNeurons; n < nNeurons; n++) {
		// 	atotInh += network[n][3]*network[n][2];
		// }
		bigAtotInh = dot2<bigType>(network, nExcNeurons, nNeurons);

		//FP time!
		reAtotExc = reAtotInh = 0.0;
		permuted = vector<NeuronState_v_n_a_s>(network);
		std::shuffle(permuted.begin(), permuted.begin() + nExcNeurons, gen);
		std::shuffle(permuted.begin() + nExcNeurons, permuted.end(), gen);
		reAtotExc = dot2<bigType>(permuted, 0, nExcNeurons);
		reAtotInh = dot2<bigType>(permuted, nExcNeurons, nNeurons);

		// errsFP.push_back({bigAtotExc - atotExc, bigAtotInh - atotInh});
		diffs.push_back(boost::array<double, 3>());

		diffs[diffs.size()-1][0] = static_cast<double>(reAtotExc - bigAtotExc);
		diffs[diffs.size()-1][1] = static_cast<double>(reAtotInh - bigAtotInh);
	...
	if (SAVE_SIMULATION) {
		...
		fileNameStr = "./results/" + sdt + "FP.txt";
		writeFPErrorsToFile(diffs, fileNameStr);
	}
